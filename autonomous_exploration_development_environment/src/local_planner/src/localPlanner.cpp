...
int readPlyHeader(FILE *filePtr)
{
  ...
  while (strCur != "end_header") {
    ...
    strLast = strCur;
    strCur = string(str);
    //fscanf read the file one by one
...
void readStartPaths()
{
  ...
  if (filePtr == NULL) {
    printf ("\nCannot read startPaths.ply, exit.\n\n");
    exit(1);
  }
...
int main(int argc, char** argv)
{
  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      //!path scale to fit the path generation file
      ...
      //goal point change to the body coordination
      ...
      // choose whether use speed to scale the path
        ...
        for (int i = 0; i < plannerCloudCropSize; i++) {
          ...
          if (dis < pathRange / pathScale && (dis <= (relativeGoalDis + goalClearRange) / pathScale || !pathCropByGoal) && checkObstacle) { 
            for (int rotDir = 0; rotDir < 36; rotDir++) {
              ...
              //condition 1 angDiff > dirThre && !dirToVehicle
              //condition 2 fabs(10.0 * rotDir - 180.0) > dirThre && fabs(joyDir) <= 90.0 && dirToVehicle
              //condition 3 (10.0 * rotDir > dirThre && 360.0 - 10.0 * rotDir > dirThre) && fabs(joyDir) > 90.0 && dirToVehicle
          ...
          if (dis < diameter / pathScale && (fabs(x) > vehicleLength / pathScale / 2.0 || fabs(y) > vehicleWidth / pathScale / 2.0) && (h > obstacleHeightThre || !useTerrainAnalysis) && checkRotObstacle) {
            ...
            float angObs = atan2(y, x) * 180.0 / PI;//弧度
            if (angObs > 0) {
