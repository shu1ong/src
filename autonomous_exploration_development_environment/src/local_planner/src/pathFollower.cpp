...
int main(int argc, char** argv)
{
  ...
  //` ros topic pubish and subscribe
  ...
  //`solve the conflict between the autonomymySpeed and the max speed
  if (autonomyMode) {
  ...
  while (status) {
    ...
    if (pathInit) {
      //todo vehicleX - vehicleXRec == 0 ? 
      //`reverse TF from global to body
      ...
      //`cal the dis to the end of the path 
      ...
      float disX, disY, dis;
      //`filter out points out of lookAheadDistance
      //todo lookAheadDistance
      ...
      */
      ...
      disY = path.poses[pathPointID].pose.position.y - vehicleYRel;
      dis = sqrt(disX * disX + disY * disY);
      float pathDir = atan2(disY, disX);//` the unit is rad
      ...
      if (!navFwd) {
        ...
        if (dirDiff > PI) 
          dirDiff -= 2 * PI;
          joySpeed2 *= -1;//`change to the diraction (back)
      }

      //!cal the yawRate(yawSpeed)
      //` when to stop can take a different yawSpeed 
      if (fabs(vehicleSpeed) < 2.0 * maxAccel / 100.0) 
        vehicleYawRate = -stopYawRateGain * dirDiff; //` YawRate nagetive to the dirDiff
      else 
        vehicleYawRate = -yawRateGain * dirDiff;
      
      //` limi the max YawRate
      ...
      //!slow down in narrow space,slowDwnDisThre determine the safety distance
      if (pathSize <= 1) {
        joySpeed2 = 0;
      ...
      //`at any time joySpeed3 = joySpeed2
      ...
      //` joyspeed3&2 not depend on joyspeed --> the topic /speed is not nessessary
      // cout << "joySpeed2:"<< joySpeed2 << endl;
      // cout << "joySpeed3:"<< joySpeed3 << endl;
      


      
      //! the value of /100 is the ros::rate
      //`the ros::rate = 100 => the DeltaT = 0.01s => DeltaSpeed = maxAccel*DeltaT = 0.01
      // for <indoor> environment the maxAccel = 2.5
      //`speed dynamic follower
      ...
      //todo why need to skip a certain pub?
      pubSkipCount--;
      if (pubSkipCount < 0) {
